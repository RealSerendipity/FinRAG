...
| Wave | 标题 | 状态 | 核心指标 |
|---|---|---|---|
| 0 | 基础能力（LLM dispatch、项目脚手架） | ✅ 已交付（待 live verify） | — |
| 1a | Postgres + pgvector schema + 单 embedding provider | ⏳ 下一个 | schema migration 幂等；`embed()` 返回向量 |
...
| 1d | OpenRouter 作为第 4 个 provider（cloud open-weight） | ⏳ | `LLM_PROVIDER=openrouter` round-trip |
...
| LLM（cloud open-weight） | OpenRouter（Llama 3.3 / Qwen / DeepSeek free tier）— *planned, Wave 1d* | Together、Groq |
