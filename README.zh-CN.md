...
- **前后端分离 Web 栈** —— Next.js 前端在服务端代理 FastAPI 请求（包括原样转发 SSE），
  API token 不会进入浏览器；两个服务都按独立 Vercel Project 组织。
...
> **在线参考：** [www.realserendipity.org/finrag](https://www.realserendipity.org/finrag/)。

本地分别启动 FastAPI 和 Next.js：
...
# FINRAG_API_TOKEN 要与后端 API_TOKEN 使用同一个值。
npm install
...
`docker compose -f infra/compose.yaml up`（[`infra/Dockerfile`](infra/Dockerfile) /
[`infra/compose.yaml`](infra/compose.yaml) 在一个容器里运行 FastAPI 和 Streamlit），
再用你自己的反向代理 / TLS（或平台自带的 HTTPS）暴露即可。

在 Vercel 中需要从同一个 GitHub 仓库导入两次：

| Vercel Project | Root Directory | 配置 |
| --- | --- | --- |
| Backend | `backend` | FastAPI 入口位于 `backend/pyproject.toml`；函数配置位于 `backend/vercel.json` |
| Frontend | `frontend` | Vercel 自动识别 Next.js；设置 `FINRAG_API_URL` 与 `FINRAG_API_TOKEN` |

后端还需要 provider、Neon、QStash 以及公网 callback 相关环境变量。Vercel
配置文件保留在 Vercel 实际构建的服务根目录中；`infra/` 只保存仓库管理的部署资源。
...
1. 在 `backend/src/ingest.py` 写一个 `chunk_<name>(text, ...) -> list[str]`(若需要 parent 上下文则 `-> list[(child, parent)]`)。
...
```text
frontend/            # Next.js UI 与仅服务端可见的 FastAPI 代理
backend/
  src/
