...
| Guardrails（Wave 6，计划中） | NVIDIA NemoGuard + custom regex/PII | — |
| MCP（Wave 6，计划中） | official `mcp` Python SDK as server | — |
...
src/
  ...
  guardrails.py      # 输入 / 输出过滤                                （Wave 6，计划中）
  mcp_server.py      # 将工具暴露为 MCP server                        （Wave 6，计划中）
...
eval/                # 评测问题集、agent 套件、指标、报告
experiments/         # 消融脚本
runs/                # agent 运行 trace，每次一份 JSONL（gitignored）
edge/                # Cloudflare Worker 源码                        （Wave 7，计划中）
tests/               # pytest 用例
