...
**评测配置**(两列共用):`fixed` 分块 · 38 条题目 · 生成模型
`meta/llama-3.3-70b-instruct` · judge `nvidia/llama-3.3-nemotron-super-49b-v1` ·
temperature 0。两列之间唯一变量是**检索方式**:Wave 2 = dense,Wave 3 = hybrid RRF + cross-encoder 重排。
...
| 3e | query rewriting(欠定查询) | HyDE recall@10 0.622→**0.676**(+0.054);multi-query −0.017 | HyDE 可选,默认关 |
