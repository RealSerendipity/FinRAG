...
- Judge provider: `nvidia` / model: `meta/llama-3.1-70b-instruct` (fallback: `gemini/gemini-2.5-flash-lite`)
...
- **citation validity** (all items, structural): 0.84
- **faithfulness** (judged 32/36): 0.88
...
|---|---|---|---|---|---|---|---|---|---|
| numeric | 8 | 0.57 | 0.71 | 0.40 | 0.47 | 0.62 | 0.80 | 1.00 | 0.80 |
| table | 8 | 1.00 | 0.90 | 0.94 | 0.86 | 1.00 | 1.00 | 1.00 | 1.00 |
| cross-document | 8 | 0.88 | 0.79 | 0.62 | 0.62 | 0.75 | 0.57 | 0.86 | 0.57 |
| reasoning | 7 | 1.00 | 0.83 | 0.74 | 0.71 | 1.00 | 1.00 | 1.00 | 0.86 |
| consistency | 7 | 0.83 | 0.72 | 0.45 | 0.52 | 0.86 | 1.00 | 1.00 | 1.00 |
...
| `aapl-fy25-cash-and-securities` | numeric | no | 1.00 | 0.11 | 0.30 | no | no | no | 1 |
...
| `xdoc-services-growth` | cross-document | yes | 0.67 | 0.50 | 0.56 | yes | no | yes | 6 |
| `xdoc-tax-rate-change` | cross-document | yes | 0.50 | 1.00 | 0.64 | yes | no | no | 4 |
| `xdoc-dividend-change` | cross-document | yes | 0.50 | 0.50 | 0.39 | yes | yes | no | 2 |
...
| `reas-fy25-us-tariffs` | reasoning | yes | 1.00 | 0.33 | 0.50 | yes | yes | yes | 2 |
...
| `reas-supply-chain-concentration` | reasoning | yes | 0.33 | 0.33 | 0.23 | yes | yes | no | 3 |
...
| `cons-fy25-iphone-paraphrase` | consistency | yes | 1.00 | 0.33 | 0.57 | yes | yes | yes | 2 |
...
**Cited chunk_ids.** [1450]

**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer matches the cited chunk.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer directly matches cited data.
...
**Cited chunk_ids.** [1463]

**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer directly matches cited chunk.
...
**Answer.** The total of Apple's cash, cash equivalents and marketable securities as of the end of fiscal year 2025 is $160,600

**Cited chunk_ids.** [291]

**Judge (primary).** faithful=no, relevant=yes, correct=no — Unsupported total value.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer matches expected value and is supported by cited chunk.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer matches expected value.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Matches the cited chunk.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer matches expected value and is supported by cited chunks.
...
**Answer.** iPhone net sales increased during 2025 compared to 2024 due to higher net sales of Pro models.

**Cited chunk_ids.** [259]

**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer accurately reflects the increase in iPhone net sales and the reason for the increase.
...
**Judge (primary).** faithful=no, relevant=no, correct=no — Answer does not address the question about R&D expense evolution.
...
**Cited chunk_ids.** [259, 1450]

**Judge (primary).** faithful=no, relevant=yes, correct=yes — ANSWER omits specific sales values present in CITED CHUNKS.
...
**Answer.** Apple's effective tax rate decreased from 24.1% in 2024 to 15.6% in 2025, primarily due to a lower effective tax rate on foreign earnings and the impact of changes in unrecognized tax benefits.

**Cited chunk_ids.** [262, 263]

**Judge (primary).** faithful=no, relevant=yes, correct=no — Incorrectly attributed the decrease in effective tax rate to a lower effective tax rate on foreign earnings and changes in unrecognized tax benefits, rather than the State Aid Decision.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=no — Missing information about the 2024 dividend increase.
...
**Cited chunk_ids.** [1445, 252, 1499, 310]

**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer matches expected values for share repurchase programs and actual repurchases.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer directly quotes the cited chunk.
...
**Answer.** Apple's fiscal 2024 effective tax rate was higher than the U.S. statutory federal income tax rate due primarily to a one-time income tax charge of $10.2 billion, net, related to the State Aid Decision and state income taxes, partially offset by a lower effective tax rate on foreign earnings, the impact of the U.S. federal R&D credit, and tax benefits from share-based compensation.

**Cited chunk_ids.** [1453]

**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer directly quotes the cited chunk with minor rephrasing.
...
**Answer.** New tariffs were announced on imports to the U.S., including additional tariffs on imports from China, India, Japan, South Korea, Taiwan, Vietnam and the European Union, posing risks of significant disruptions to global supply chains and adverse impacts on the Company's business and results of operations.

**Cited chunk_ids.** [195, 195]

**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer accurately describes new tariffs and their risks.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=no — Missing U.S. partners and facilities.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Matches the total net sales figure for 2024.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Matches expected value.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer directly quotes the effective tax rate from the cited chunk.
...
**Judge (primary).** faithful=yes, relevant=yes, correct=yes — Answer directly matches cited total gross margin percentage.
...
Acceptance gate: two runs of the same harness should agree within ±0.02. This report is the error-free run; the rerun hit one transient Neon connection drop (`cons-fy25-iphone-paraphrase`, SSL EOF), which counts that item as citation-invalid and shrinks the judged denominator by one.

| metric | this report | rerun | abs Δ |
|---|---|---|---|
| recall@5 | 0.6619 | 0.6619 | 0.0000 |
| recall@10 | 0.7962 | 0.7962 | 0.0000 |
| MRR | 0.6436 | 0.6436 | 0.0000 |
| nDCG@10 | 0.6510 | 0.6510 | 0.0000 |
| doc coverage@10 | 1.0000 | 1.0000 | 0.0000 |
| citation validity | 0.8421 | 0.8158 | 0.0263 — transient DB error = 1/38; excluding that item, 0.0000 |
| faithfulness | 0.8750 | 0.8667 | 0.0083 |
| answer relevancy | 0.9688 | 0.9667 | 0.0021 |
| correctness | 0.8438 | 0.8667 | 0.0229 — one judge flip |

Findings:

- **Retrieval metrics are fully deterministic** — identical to 4 decimal places across all four runs performed during this wave (HNSW search over the same embeddings).
- **Judge-metric granularity bounds the gate.** With n≈31 judged answers, a single verdict flip moves a metric by ~0.032, so the ±0.02 gate effectively demands zero flips. Post-fix variance is within one flip per metric.
- **One systematic judge instability was found and fixed.** With the initial rubric, two correctness verdicts flipped between runs (Δ 0.065): the judge sometimes penalized answers for omitting secondary context (YoY growth) that `expected_answer` mentions. The rubric now states explicitly that omitting secondary context the question did not ask for is not incorrect; remaining variance is provider-side non-determinism at temperature 0.
- **Implication for Wave 3:** retrieval deltas are trustworthy as-is; judge-metric deltas below ~0.05 should be treated as noise (or re-run to confirm) when ranking retrieval variants.
